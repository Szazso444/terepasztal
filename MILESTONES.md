...
## 14. v0.8.0 — track classes, body model, crafting, electrification, towns, junctions

- **Track classes** (`src/world/track.ts`, `src/world/trackGeom.ts`): Regular (`n = 1`) and High-speed (`n = 2`); curve radius `n − 0.5`, curve and switch footprint `n × n`, cost `n × 1.5`. Wide pieces are generated by sampling each route and clipping it to the tiles it crosses, so every member tile keeps a cardinal edge pair and the tile graph, pathfinding and traffic sections are unchanged; `TrackGraph.resolveRoutes` picks the route through a switch throat. Three crossings, a transition piece (the only join between classes, enforced at placement), `curveFactor = min(1, k√R)`. Costs from the ratio matrix in `track.json` times `trackCostScale`; starting wood/stone/iron derive from it times `startingResourceScale`.
- **Body model** (`src/sim/body.ts`): sizes small / medium / large (1 / 2 / 3 tiles), plans rigid, engine + tender, Garratt, Meyer; bogies at fixed arc offsets, the body posed from its outer bogies and shifted (≤ 0.40) to centre on the track, residual gap and bogie slides measured every tick. Consists roll out of a virtual run inside the shed; occupancy samples the whole consist.
- **Rendering**: 24 facings, 13 drawn (the rest mirror `tx ↔ ty`), runtime rotation of the residual projected angle, shared bogie sprites under medium and large bodies; every model redrawn at its size, six new locomotives, service carts, brake van, boxcar.
- **Compatibility and bans** (`src/sim/compat.ts`): per model and class a traversal of a straight–arc–straight reference path records peak fore-and-aft slide, sideways shift, residual gap (limit 0.25 per tile of body) and centre-bogie offset (limit 0.35); large stock is barred from regular track by rule. Consist access filters every path search; depot gates are checked for class, an onward tile and room (`Fleet.gateReport`); the picker greys barred stock with the reason; refusals name the vehicle and the first tile it cannot pass. Rails may run through the shed.
- **Crafting** (`src/gacha/crafting.ts`, `src/data/crafting.json`): unlock a recipe with money (three cards, keep one; known cards refund), craft with resources, failure by quality with partial refund; unlimited copies, spares level a model; the gacha code is parked.
- **Electrification** (`src/sim/catenary.ts`): third rail, catenary, HV catenary laid over track; live within six tiles of a Substation (a pole-grid node) that is powered; substations pass a set number of units per second and every train under one shares it; Hydro Plant beside water. Electric traction needs live wire under the head tile; older saves get catenary over the rails their poles powered.
- **Towns** (`src/sim/houses.ts`): townhouses build over two days in three stages, hold 10 / 25 / 50 by level, grow one resident per two days while fed, spawn near track and houses at 80 % of capacity with a traffic multiplier, upgrade when full; wheat per head doubled.
- **Junctions** (`src/sim/junctions.ts`): switches and crossings within six rail tiles of each other form a junction; a 60 s window of train count, total and longest wait sets Info / Minor / Major / Critical with cooldowns; the notice pans to the junction and tints the contested tiles.
- **Semaphores** (`src/art/structures.ts`): home and distant arms animated between danger, caution and clear; `src/sim/signals.ts` computes block aspects and token holders for the signalling levels.
