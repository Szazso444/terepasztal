  ...
  update(trains: Train[], all: boolean, force = false) {
    const key =
      ...
      trains
        .map(
          (t) =>
            `${t.id}:${t.state}:${Math.round(t.speed * 20)}:${Math.round(t.coal)}:${Math.round(t.water)}:${Math.round(t.oil)}:${Math.round(t.battery)}:${t.fuelPreference}:${t.lastTrip?.endedAt ?? 0}:${Math.round(t.weight)}:${t.locos.map((l) => `${l.mode}${l.engaged ? 1 : 0}`).join()}:${t.wagons.map((w) => `${w.cargo}${Math.round(w.amount)}`).join()}`,
        )
        .join('|');
