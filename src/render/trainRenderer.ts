...
import { locoFrame, wagonFrame, loadKind, bogieFrame } from '../art/frames';
import {
  facingOf,
  DRAWN_FACINGS,
  mirrorFacing,
    ...
    for (const t of trains) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        segments.forEach((seg, si) => {
          ...
          const style = (isLoco ? t.locos[i].def : t.wagons[i - t.locos.length].def).bogieStyle;
          ...
          if (c.spec.drawBogies)
            seg.bogies.forEach((b, k) => {
              ...
              this.pose(
                bs,
                (f) => bogieFrame(this.atlas, style, b.kind, seg.part, f),
                bx,
                by,
                ba,
                14,
