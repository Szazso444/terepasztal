...
## Chat payload and asset delivery

Keep generated images and other large artifacts in the repository. The user reviews
them in the codebase: report paths and concise text summaries instead of embedding
images, base64 data, or large file contents in chat. Keep tool output bounded and
recover session history by extracting text only. Save asset progress in
`assets/source/base-v1/RESUME.md` so work can resume without replaying media-heavy
conversations. This reduces payload size; it does not change the app's request limit.
...
- `tools` node-side build tooling (the atlas packer); `tools/asset-pipeline` is the Python photo -> 3D -> sprite
  pipeline (ComfyUI, Blender), with its own `AGENTS.md`
